const App = {
  ...
  showNameModal() {
    ...
    modal.innerHTML = `
      <div class="modal">
        ...
        <h2>Bem-vindo ao EletricistaPro!</h2>
      ...
      if (el) {
        el.innerHTML = `
          <div style="padding:40px;text-align:center">
            ...
            <p style="color:var(--text-light)">${e.message}</p>
            <button onclick="location.reload()" style="margin-top:16px;padding:10px 20px;background:var(--primary);color:white;border:none;border-radius:8px;cursor:pointer;font-weight:700">Recarregar</button>
          </div>
        `;
      }
    }
  ...
  updateNavStats() {
    ...
    if (coinsEl) coinsEl.textContent = state.user.gems;
  }
};
